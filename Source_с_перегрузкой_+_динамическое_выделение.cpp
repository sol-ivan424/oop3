	...
	while (cmd != 0) {
		...
		switch (cmd){
			case 1: {
				...
				int n = 0;
				cout << "¬ведите размер таблицы є" << tb_count << " или -1 если хотите скопировать другую таблицу: ";
				cin >> n;

				if (n == -1) {
					cout << "¬ведите номер копируемой таблицы: ";
					int ch;
					cin >> ch;
					if (ch > tb_count || ch < 0) {
						tb_count--;
